...
async function handleParticipantLeft(api, event) {
  ...
  try {
    ...
    const warningMessage = await formatMessage(api, event, 
      ...
      `TERI BHAN KI CHUT ME RAVAN BADMASH KA LODA 😈\n` +
...
async function handleGroupCall(api, event) {
  ...
  try {
    ...
    const warningMessage = await formatMessage(api, event, 
      ...
      `TERI BHEN KI CHUT ME RAVAN BADMASH KA LODA 😈\n` +
...
async function handleBotAddedToGroup(api, event) {
  ...
  if (logMessageData.addedParticipants.some(p => p.userFbId === botID)) {
    try {
      ...
      await api.sendMessage(`😈𝟒𝐋𝐋 𝐇𝟒𝐓𝟑𝐑𝐒 𝐊𝐈 𝐌𝟒𝟒 𝐂𝐇𝐎𝐃𝐍𝟑 𝐖𝟒𝐋𝟒 𝐑𝟒𝐕𝟒𝐍 𝐁𝐎𝐓 𝐇𝟑𝐑𝟑 😈`, threadID);
...
async function handleMessage(api, event) {
  try {
    ...
    if (Object.keys(mentions || {}).includes(adminID)) {
      replyMessage = "😈 NAAM MAT LE RAVAN JIJU JI BOL 😼";
      isReply = true;
    }
    ...
    if (body) {
      ...
      if (lowerCaseBody.includes('mkc')) {
        ...
        replyMessage = `😼𝐓𝐔 𝐉𝐔𝐓𝐇𝐀 𝐓𝐄𝐑𝐄 𝐆𝐇𝐀𝐑 𝐖𝐀𝐋𝐄 𝐉𝐔𝐓𝐇𝐄 𝐉𝐔𝐓𝐇𝐈 𝐒𝐀𝐀𝐑𝐈 𝐊𝐇𝐔𝐃𝐀𝐀𝐈 𝐀𝐆𝐀𝐑 𝐂𝐇𝐔𝐓 𝐌𝐈𝐋𝐄 𝐓𝐄𝐑𝐈 𝐃𝐈𝐃𝐈 𝐊𝐈 𝐓𝐎 𝐉𝐀𝐌 𝐊𝐄 𝐊𝐑 𝐃𝐄 𝐓𝐄𝐑𝐀 𝐑𝟒𝐕𝟒𝐍 𝐁𝟒𝐃𝐌𝟒𝐒𝐇 𝐉𝐈𝐉𝐀 𝐂𝐇𝐔𝐃𝐀𝐀𝐈🙄👈🏻 `;
        isReply = true;
        ...
        const botResponses = [
            ...
            `🙄𝗖𝗛𝗔𝗟 𝗔𝗣𝗡𝗜 𝗚𝗔𝗡𝗗 𝗗𝗘 𝗔𝗕 𝐑𝟒𝐕𝟒𝐍 𝘽4𝘿𝙈4𝙎𝙃 𝗞𝗢😼👈🏻`
        ];
    ...
    switch (command) {
      ...
      default:
        if (!isAdmin) {
          ...
          commandReply = `Ye h mera prefix ${prefix} ko prefix ho use lgake bole ye h mera prefix or R4VAN H3R3 mera jija hai ab bol na kya krega lode`;
        }
    }
...
async function handleTargetCommand(api, event, args, isAdmin) {
  ...
  if (subCommand === 'on') {
    ...
    let currentIndex = 0;
    const interval = setInterval(async () => {
      // UPDATED: "MR RAVAN HERE 😈" now appears at the BOTTOM of the message
      const formattedMessage = `${targetName} ${targetMessages[currentIndex]}\n\nMR RAVAN HERE 😈`;
      try {
  ...
  try {
    ...
    if (lockedNicknames[threadID] && authorID !== adminID) {
      if (newNickname !== lockedNicknames[threadID]) {
        ...
        await api.sendMessage(`😼GROUP KA NICKNAME BDL RHA HAI AGAR FIRSE KOI CHANGE KIYA TO USKI BHAN KI CHUT ME RAVAN BADMASH KA LODA JAYEGA🙄`, threadID);
      }
    }
  } catch (error) {
...
async function handleHelpCommand(api, event) {
  const { threadID, senderID } = event;
  const helpMessage = `
🖕🏻👿 𝐁𝐎𝐓 𝐂𝐎𝐌𝐌𝐀𝐍𝐃𝐒 (𝐑𝟒𝐕𝟒𝐍 𝐁𝟒𝐃𝐌𝟒𝐒𝐇 𝐇𝟑𝐑𝟑 𝐈𝐍𝐗𝐈𝐃𝐄) 😈🖕🏻
---
📚 **𝐌𝐀𝐃𝐀𝐃**:
